...
**重要提醒**：
1. **识别行情类型**：首先判断当前是单边行情、震荡行情还是突破行情，不同行情用不同策略
2. **震荡行情**：
   - 开仓：在支撑位做多、阻力位做空（高抛低吸）
   - 平仓：盈利≥30点立即平仓！震荡中无法吃大行情！
   - 目标：10-30点，快进快出
3. **单边行情**：
   - 开仓：顺势追涨/杀跌，不怕追高
   - 平仓：趋势不变可持有，但盈利≥30点要警惕反转
   - 目标：20-50点，跟随趋势
4. **回撤保护**（所有行情都适用）：
   - 回撤>50%：**必须平仓**！
   - 回撤>40%：**优先平仓**！
...
4. **必须先识别行情类型**（单边/震荡/突破），然后根据类型选择对应策略
5. 震荡行情：目标10-20点，快进快出；单边行情：目标20-50点，跟随趋势
