...
// K线预测专用提示词 - 伦敦市场（独立预测）
const KLINE_PREDICTION_PROMPT_LONDON = `你是一个专业的价格预测分析师，精通主力操盘手法和市场微观结构。请根据提供的伦敦现货白银K线数据预测后续100个价格点（每个点代表1分钟后的价格）。
...
3. **预测100个价格点**：
   - 每个价格点代表未来1分钟后的预测价格（共100分钟）
   - **预测要符合主力操盘逻辑**：
     * 如果主力在拉升，价格应逐步走高，可能分阶段（10-20分钟）推进
     * 如果主力在洗盘，价格可能先抑后扬或震荡下行，然后反弹
     * 如果主力在出货，价格可能高位震荡或缓慢下行，可能有诱多反弹
   - 价格应该合理连续，符合主力操作节奏
   - 预测要体现趋势的延续、反转或震荡整理
   - 价格范围要合理（参考最近的价格波动和主力操作幅度）
   - 100分钟的预测要有阶段性变化，不要单调线性
...
- 价格变化要平滑合理
- **预测要体现主力操盘的真实逻辑，不要简单的线性上涨或下跌**
- 主力通常会在整数关口或关键位置进行试探、洗盘、突破等操作
- 100个点要体现阶段性变化（如：震荡-突破-回踩-拉升等）
`;

// K线预测专用提示词 - 国内市场（参考伦敦预测）
const KLINE_PREDICTION_PROMPT_DOMESTIC = `你是一个专业的价格预测分析师，精通主力操盘手法和市场微观结构。请根据提供的国内白银主力K线数据和伦敦现货白银的预测价格，预测后续100个价格点（每个点代表1分钟后的价格）。
...
4. **预测100个价格点**：
   - 每个价格点代表未来1分钟后的预测价格（共100分钟）
   - **预测要符合主力操盘逻辑**：
     * 如果主力在拉升，价格逐步上涨，可能在关键整数位短暂回调
     * 如果主力在洗盘，价格可能震荡或假跌，但整体跟随伦敦方向
     * 如果主力在出货，价格可能高位震荡，逐步走低
   - 价格应该合理连续，符合主力操作节奏
   - 整体涨跌趋势要与伦敦市场预测保持一致
   - 价格范围要合理（参考最近的价格波动和主力操作幅度）
   - 100分钟的预测要有阶段性变化，体现主力的多步骤操作
...
- 价格变化要平滑合理，但要体现主力操作的节奏感
- 100个点要体现阶段性（如：震荡-突破-回踩-拉升-整理等多阶段）
`;
